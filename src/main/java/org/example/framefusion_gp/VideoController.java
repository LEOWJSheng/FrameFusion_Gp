...
public class VideoController {
    ...
    @FXML private Slider timelineTrack;
    ...
    private void setupDragAndDropOnVideoTrack() {
        ...
        videoTrack.setOnDragDropped(ev -> {
            ...
            timelineTrack.setMax(totalDuration);
    ...
    private void setupPlayheadDragging() {
        // minimal drag-to-scrub on the Slider
        timelineTrack.setOnMouseDragged(e -> {
            double trackW = timelineTrack.getWidth();
            double newX = e.getX();
            newX = Math.max(0, Math.min(newX, trackW));
            double pct = newX / trackW;
            double newTime = pct * totalDuration;
            timelineTrack.setValue(newTime);
            updatePreviewAtTime(newTime);
            e.consume();
        });
    }
    ...
    private void onExport() {
        ...
        File dir = new File("exported/videos");
        ...
        new Thread(() -> {
            try {
                ...
                for (int i = 0; i < frames; i++) {
                    double t = i / fps;
                    int w = ((int)Math.ceil(mediaPlayerPane.getWidth()/2.0) * 2);
                    int h = ((int)Math.ceil(mediaPlayerPane.getHeight()/2.0) * 2);
                    WritableImage wi = new WritableImage(w, h);
                    CountDownLatch latch = new CountDownLatch(1);
                    Platform.runLater(() -> {
                        updatePreviewAtTime(t);
                        mediaPlayerPane.snapshot(params, wi);
                        latch.countDown();
                    });
                    latch.await();
                ...
                SnapshotParameters thumbParams = new SnapshotParameters();
                thumbParams.setFill(Color.WHITE);

                WritableImage thumbImg = new WritableImage(
                        (int)mediaPlayerPane.getWidth(),
                        (int)mediaPlayerPane.getHeight()
                );

                Database.insertVideo(outFile.getAbsolutePath());
    ...
    private void switchToGallery() {
        try {
            FXMLLoader l=new FXMLLoader(getClass().getResource("videoPageController.fxml"));
            Parent r=l.load();
